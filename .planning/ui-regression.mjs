...
const screenshotPath = path.resolve(".planning/ui-room-preview.png");
...
try {
  ...
  await page.getByText("02", { exact: true }).waitFor();
  await page.getByText("03", { exact: true }).waitFor();
  ...
  if (await dialog.locator("select").count() !== 2) throw new Error("Time selectors missing");
