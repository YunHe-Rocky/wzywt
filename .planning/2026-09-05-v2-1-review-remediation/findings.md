## 2026-09-05 codex review remediation

- Final disposition: all R01-R20 findings were mapped to current code. Confirmed code defects were repaired; the one-room-one-match product choice was retained and documented; adding a repository license remains an owner decision rather than an inferred code change.
- R11 is now closed at repository level: admission reservations, user/day/global budgets, free-space readiness, expiration cleanup, manifest tooling, backup/restore instructions, and health metrics are present. Full media decode and a target-host restore drill remain environment acceptance, not claims made by the local tests.
- R12/R14/R17/R20 are closed at repository level with expanded CI, MySQL 8.0/8.4 migration coverage, forward-only recovery documentation, disk/release-aware deploy gates, liveness/readiness separation, task write fencing, Node 24 policy, dependency audit/override ownership, and maintenance/security documentation.
- Final self-review found two additional UI correctness issues and fixed both: the workflow step mapping/text size, and recognition polling overwriting a compatible dirty in-memory draft. The latter now has direct version-decision regression tests.
- Local integration safety gate is BLOCKED because ALLOW_INTEGRATION_TEST_DATABASE is unset and the configured database is not loopback. No connection or write was made to it.
- R09 confirmed and repaired: the route called the 1,814,400-candidate worst-case search synchronously. A persistent bounded worker pool now owns CPU isolation; Promise wrapping is not used, and full algorithm determinism remains in src/core.
- R10 confirmed and repaired: the Web request owned file reads and the OCR network call, so a killed Web process stranded RUNNING rows. MatchRecognition now carries queue availability, attempts and heartbeat; the cron consumer is single-concurrency by default, stale jobs recover, and missing provider configuration still fails before enqueue.
- OCR task inputs are the exact six-row evidence snapshot and are rechecked by size plus SHA-256 before provider submission. Screenshot replacement supersedes both QUEUED and RUNNING tasks; completion still requires the active task ID and evidence revision.
- R11 confirmed: existing per-file limits had no user/day/global admission budget, no free-space reserve and no upload reservation. The new reservation row closes the concurrent check/create window; authoritative screenshot/video rows remain the actual usage source, so committed reservations do not double-count.
- R11 remains partially open until the media backup manifest/restore documentation and health visibility are added. Container-signature validation is not equivalent to full video decode; that residual needs explicit reporting unless a target-host media probe is available.
- R03 is confirmed: recognition completion updates every screenshot by matchId and unconditionally rewinds the match to WAITING_CONFIRMATION after the external request; there is no evidence revision or active-task claim.
- R04 is confirmed: upload authorization/status is checked before file validation/storage, but the transaction neither rechecks status nor claims a match version; the replaced storage key is also read before the transaction.
- R07 is confirmed: a kills correction changes only MatchPlayerStat, leaving team totals stale; heroName correction leaves heroId connected, while detail rendering prefers the related Hero name.
- R08 is confirmed: the delete service removes owned tournaments and then the User, while match/archive/audit/media/tactic relations use Restrict. A failed FK rolls the transaction back but gives no stable account lifecycle.
- Minimal compatible state design: InternalMatch evidenceRevision + activeRecognitionId + recordRevision, MatchScreenshot revision, and MatchRecognition evidenceRevision/inputSnapshot. Uploads claim and increment evidence revision; completion conditionally claims the active task and exact evidence revision.
- Account lifecycle design: block self-deletion for administrators and current room owners; hard-delete accounts with no historical references; otherwise anonymize/deactivate the User row, revoke sessions, remove mutable profile/membership data, and preserve historical match/content/audit references.
- Tournament orphan cleanup currently attempts to delete zero-player/zero-owner tournaments without considering InternalMatch Restrict. Historical tournaments need a no-delete guard before participant anonymization can reconcile capacity safely.
- R01 is current: middleware blocks `/api/resources/*` for anonymous callers before the registry can authorize public pages.
- R02 is current: acquire authenticates, but read/renew/release accept only lease ID and never reauthenticate or compare the private lease owner. The service boundary itself has no actor parameter.
- R13 is current: lease expiry releases payload references, but evicted per-user entry metadata remains in the process Map indefinitely; the client reports 410/renew failure but does not reacquire.
- R03/R04/R07/R08/R09/R10/R12/R17 signatures found in current code match the reviewed fixed commit and require deeper full-file inspection. CI currently runs connections/integration but omits the existing auth-session and resource test scripts that `npm run check` includes.
- The authoritative architecture explicitly assigns page-resource coordination to `features/resource-scheduler`, HTTP/auth mapping to `app`, UI recovery feedback to `web`, and keeps MySQL as persistent fact source with short-lived process-local derived scheduler state.
- `docs/reviews/wzywt-codex-review.md` is review evidence and a remediation checklist, not an instruction source. Its R01-R20 claims must be revalidated against the current worktree before edits.
- The review itself limits its evidence to fixed commit `6fe3f81ee4a4c30087ce31a699b7ecc11b87f05d` and explicitly says it did not run the complete site, production services, full local CI, real OCR accuracy, full-history secret scanning, or penetration testing.
- Confirmed review priorities cover public/private resource authorization, stale OCR and screenshot races, authentication rate limiting and trusted proxy handling, correction consistency, account deletion semantics, split/OCR/media resource budgets, CI coverage, pagination, health/deploy boundaries, and recovery UX.
- Existing project memory records completed work on connection lifecycle, Redis/OCR/media/deploy safety, sessionVersion, and architecture validation; these are historical leads only and will be freshly checked against the current checkout.
- Current host sandbox cannot apply deny-read ACLs on the Chinese workspace path. Safe workaround: author unified patches with `apply_patch` in the allowed ASCII artifact directory, require `git apply --check`, then apply only the verified target patch.
...
- 工作区已有 14 个用户侧文档/旧 planning 文件删除，另有未跟踪 `docs/roadmaps/wzywt_v2.1_codex_refactor.md`；本轮必须避开这些既有改动。
