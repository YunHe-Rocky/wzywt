...
export default function AdminHeroesPage() {
  ...
  const filtered = heroes.filter((h) => {
    ...
    if (filter === "changed") {
      ...
      return defaultLane && h.roleType !== defaultLane;
    }
  ...
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "40px 24px" }}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        <button onClick={() => setFilter("changed")} className={filter === "changed" ? "btn-primary" : "btn-subtle"} style={{ padding: "5px 14px", fontSize: 12, borderColor: "rgba(240,192,64,0.3)" }}>
          已修正
        </button>
      </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: "1px solid var(--border)", background: "var(--bg-hover)" }}>
                ...
                <th style={{ padding: "10px 16px", textAlign: "left", fontWeight: 600, color: "var(--text-secondary)", fontSize: 11, letterSpacing: 1, whiteSpace: "nowrap" }}>当前分路</th>
                <th style={{ padding: "10px 16px", textAlign: "left", fontWeight: 600, color: "var(--text-secondary)", fontSize: 11, letterSpacing: 1, whiteSpace: "nowrap" }}>修正为</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((hero) => {
                  ...
                  <tr key={hero.heroId} style={{ borderBottom: "1px solid var(--border-light)" }}>
                    ...
                    <td style={{ padding: "4px 16px" }}>
                      <select
                        ...
                        disabled={saving === hero.heroId}
                        style={{
                          padding: "4px 8px", fontSize: 12, width: 110,
                          opacity: saving === hero.heroId ? 0.5 : 1,
                        }}
                      >
                        {ROLES.map((r) => (
                      ...
                      {saving === hero.heroId && <span style={{ fontSize: 11, color: "var(--gold)", marginLeft: 6 }}>...</span>}
                    </td>
                  </tr>
                );
              })}
