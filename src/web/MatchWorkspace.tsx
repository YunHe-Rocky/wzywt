...
interface DetailData {
  ...
  access: { canManage: boolean; isSuperAdmin: boolean; currentUserId: number };
...
export function MatchWorkspace() {
  ...
  const redTotalKills = redPlayers.reduce((sum, player) => sum + Number(player.stats.kills), 0);
  const blueTotalKills = bluePlayers.reduce((sum, player) => sum + Number(player.stats.kills), 0);
    ...
    <main className="page-shell page-shell--wide match-workspace">
      ...
      <header className="match-record-header">
        <div className="match-record-title">
          ...
          <div><h1>比赛档案</h1><strong>#{String(matchId).padStart(3, "0")}</strong></div>
      ...
      <MatchWorkflowProgress status={detail.match.status} screenshotCount={detail.match.screenshots.length} recognitionStatus={detail.match.recognition?.status} />
      ...
      <section className="match-sheet" aria-labelledby="confirm-title">
        <div className="match-section-heading">
          <div className="match-section-copy"><span className="match-section-index">01</span><div><span className="match-section-label">比赛数据</span><h2 id="confirm-title">十人数据复核</h2><p>识别结果仅作录入辅助，成员身份与比赛数据仍需人工确认。</p></div></div>
        ...
        <p className="match-result-table-hint" id="match-result-table-hint">桌面端按双方对照；窄屏会自动排成逐选手记录，无需横向滑动。</p>
      ...
      <section className="match-sheet match-finalize" aria-labelledby="submit-title">
      ...
      <div className="feature-two-columns match-secondary-grid">
        ...
        <section className="match-secondary-section"><span className="match-section-label">队内工具</span><h2>战术复盘</h2><p>仅本队成员进入对应战术室；提交前各自标注，提交后队内只读复盘。</p><div className="feature-row-actions"><Link className="btn-subtle match-team-link match-team-link--red" href={`${routePrefix}/tournaments/${tournamentId}/matches/${matchId}/tactics/red`}>红方战术室</Link><Link className="btn-subtle match-team-link match-team-link--blue" href={`${routePrefix}/tournaments/${tournamentId}/matches/${matchId}/tactics/blue`}>蓝方战术室</Link></div></section>
      </div>
