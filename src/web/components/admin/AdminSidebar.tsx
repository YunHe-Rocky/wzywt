...
export function AdminSidebar({ username }: { username: string }) {
  ...
  return (
    <aside className="admin-sidebar" style={{
      ...
      <div className="admin-brand px-4 py-3 border-b border-border-light">
      ...
      <nav className="admin-nav flex-1 px-2.5 py-2 flex flex-col gap-0.5" aria-label="后台导航">
        {NAV.map((item) => {
          const active = item.exact ? pathname === item.href : pathname.startsWith(item.href);
          return (
            <Link key={item.href} href={item.href}
              className={`admin-nav-link flex items-center gap-2 px-2.5 py-1.5 rounded-md text-[13px] no-underline transition-colors ${
      ...
      <div className="admin-footer px-2.5 py-2 border-t border-border-light">
