...
import { ROLE_LABELS, ROLES } from "@/core/game";
import { getHeroes } from "@/features/heroes/client/api";
import { addHeroPower, getHeroPowers, removeHeroPower } from "@/features/profile/client/api";
...
export function HeroPowerEditor() {
  ...
  useEffect(() => {
    getHeroPowers().then(({ data }) => {
      if (data.heroPowers) setGrouped(data.heroPowers as Record<string, HeroEntry[]>);
    });
  }, []);

  async function loadHeroes(role: string) {
    if (heroOptions[role]) { setActiveRole(role); return; }
    const { data } = await getHeroes<HeroOption>({ roleType: role });
  ...
  async function addHero() {
    ...
    if (ok) {
      const created = data as unknown as HeroEntry;
      setGrouped((prev) => ({
        ...prev,
