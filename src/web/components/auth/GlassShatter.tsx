...
export function GlassShatter({ cardSelector, onComplete }: Props) {
  ...
  useEffect(() => {
    ...
    // 阶段时序
    const crackDuration = 180;
    const burstDuration = 1050;
    const t1 = setTimeout(() => setPhase("shatter"), crackDuration);
    const t2 = setTimeout(() => {
      setPhase("done");
      onCompleteRef.current();
    }, crackDuration + burstDuration);
  ...
  return (
    <div style={{
      position: "fixed", inset: 0, zIndex: "var(--layer-transition)",
            ...
            {Array.from({ length: 20 }).map((_, i) => {
              ...
              return (
                <line key={i} x1={`${x1}%`} y1={`${y1}%`} x2={`${x2}%`} y2={`${y2}%`}
                  ...
                  style={{ animation: `crack-line-in 0.16s ease-out ${i * 0.006}s both` }} />
              );
            })}
          </svg>
        </div>
