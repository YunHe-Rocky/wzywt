...
export function CombatWall() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(false);
    if (!result.ok) return error(result.data.error || "演武动态加载失败，请先登录");
  ...
  async function publish(event: FormEvent<HTMLFormElement>) {
    ...
    setPublishing(false);
    if (!result.ok) return error(result.data.error || "发布失败");
  ...
  return <main className="page-shell page-shell--wide combat-wall">
    ...
    <section aria-labelledby="feed-title"><div className="feature-heading"><div><p className="feature-kicker">LATEST</p><h2 id="feed-title">最新动态</h2></div></div>
      {loading ? <div className="feature-empty">正在读取动态…</div> : posts.length === 0 ? <div className="feature-empty">还没有人发布复盘。</div> : <div className="combat-grid">{posts.map((post) => <article className="combat-card" key={post.id}>
        ...
        <div className="combat-card-body"><div className="feature-meta-row"><strong>{post.author.username}</strong><span>{new Date(post.createdAt).toLocaleString("zh-CN")}</span>{post.status !== "published" && <span className="feature-status">{post.status}</span>}</div><h3>{post.title}</h3><p>{post.content}</p><footer><span>赞 {post._count.likes} · 评论 {post._count.comments}</span><Link className="btn-subtle" href={`${routePrefix}/combat/${post.id}`}>查看复盘</Link></footer></div>
      </article>)}</div>}
      <div className="pagination"><button className="btn-subtle" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>上一页</button><span>{page} / {totalPages}</span><button className="btn-subtle" disabled={page >= totalPages} onClick={() => setPage((value) => value + 1)}>下一页</button></div>
    </section>
  </main>;
}
