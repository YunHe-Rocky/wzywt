...
type TacticConfirmation = { kind: "layer" | "route" } | { kind: "marker"; marker: MarkerItem } | null;
...
export function TacticBoard() {
  ...
  async function addMarkerAt(point: TacticPoint) {
    ...
    try {
      ...
      if (!result.ok) return error(apiMessage(result.data, "点位保存失败，请重试"));
      success("点位已保存"); await load();
    } catch (cause) {
      error(cause instanceof Error ? cause.message : "点位保存失败，请检查网络后重试");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function saveCurrentRoute() {
    ...
    try {
      ...
      const result = await saveRoute(tournamentId, matchId, side, activeLayer.id, input);
      if (!result.ok) return error(apiMessage(result.data, "路线保存失败，请刷新后重试"));
      success("路线已保存"); await load();
    } catch (cause) {
      error(cause instanceof Error ? cause.message : "路线保存失败，请检查网络后重试");
    } finally {
      setBusy(false);
    }
  }
  async function removeOwnRoute() {
    ...
    try {
      ...
      if (!result.ok) return error(apiMessage(result.data, "路线删除失败"));
      setConfirmation(null); success("路线已删除"); await load();
    } catch (cause) {
      error(cause instanceof Error ? cause.message : "路线删除失败，请检查网络后重试");
  ...
  async function saveLayerRange() {
    ...
    setBusy(true);
    try {
      const result = await updateLayer(tournamentId, matchId, side, activeLayer.id, { name: activeLayer.name, description: activeLayer.description, expectedUpdatedAt: activeLayer.updatedAt, ...range });
      if (!result.ok) return error(apiMessage(result.data, "图层时间保存失败，请刷新后重试"));
      success("图层时间已保存"); await load();
    } catch (cause) {
      error(cause instanceof Error ? cause.message : "图层时间保存失败，请检查网络后重试");
    } finally {
      setBusy(false);
    }
  }
  async function removeLayer() {
    ...
    try {
      ...
      if (!result.ok) return error(apiMessage(result.data, "图层删除失败"));
      setConfirmation(null); success("图层已删除"); await load();
    } catch (cause) {
      error(cause instanceof Error ? cause.message : "图层删除失败，请检查网络后重试");
    } finally { setBusy(false); }
  }
  async function removeMarker(item: MarkerItem) {
    ...
    try {
      ...
      if (!result.ok) return error(apiMessage(result.data, "点位删除失败"));
      setConfirmation(null); success("点位已删除"); await load();
    } catch (cause) {
      error(cause instanceof Error ? cause.message : "点位删除失败，请检查网络后重试");
    } finally { setBusy(false); }
  }
  ...
  return <main className={`tactic-shell tactic-shell--${side}`}>
    ...
    <section className="tactic-clock-panel" aria-label="比赛时间轴">
      <div className="tactic-clock-control">
        ...
        <input aria-label="比赛时间" aria-valuetext={formatTacticTime(clockSeconds)} type="range" min="0" max={TACTIC_CLOCK_MAX_SECONDS} step="1" value={clockSeconds} onChange={(event) => setClockSeconds(Number(event.target.value))} />
    ...
    <section className="tactic-stage-panel tactic-stage-panel--focused">
        ...
        <details className="tactic-advanced"><summary>图层与精确编辑</summary><div className="tactic-advanced-grid">
          {data.access.canDraw && !data.access.sharedAnnotationsVisible && <div className="tactic-layer-manager">
            <h3>{data.access.canManageLayers ? "管理时间图层" : "新建本队图层"}</h3>
            <label>图层名称<input value={layerName} maxLength={64} placeholder="新图层名称" onChange={(event) => setLayerName(event.target.value)} /></label>
            <div className="tactic-layer-time-inputs"><label>开始<input value={layerStart} placeholder="2:00" inputMode="numeric" onChange={(event) => setLayerStart(event.target.value)} /></label><label>结束<input value={layerEnd} placeholder="10:00" inputMode="numeric" onChange={(event) => setLayerEnd(event.target.value)} /></label></div>
            <div className="tactic-advanced-actions">
              <button className="btn-subtle" disabled={busy || !layerName.trim()} onClick={addLayer}>新建图层</button>
              {data.access.canManageLayers && <>
                <button className="btn-subtle" disabled={busy || !activeLayer} onClick={saveLayerRange}>保存时间</button>
                <button className="btn-danger" disabled={!activeLayer || busy} onClick={() => setConfirmation({ kind: "layer" })}>删除图层</button>
              </>}
            </div>
          </div>}
          <div><h3>精确坐标</h3><div className="tactic-accessible-editor"><label>X（0–1）<input disabled={!data.access.canDraw} type="number" min="0" max="1" step="0.01" value={coordinate.x} onChange={(event) => setCoordinate({ ...coordinate, x: event.target.value })} /></label><label>Y（0–1）<input disabled={!data.access.canDraw} type="number" min="0" max="1" step="0.01" value={coordinate.y} onChange={(event) => setCoordinate({ ...coordinate, y: event.target.value })} /></label><button className="btn-subtle" disabled={!data.access.canDraw || !activeLayer || busy} onClick={addAccessiblePoint}>按坐标添加</button></div><button className="text-action" disabled={!data.access.canDraw || !ownRoute || busy} onClick={() => setConfirmation({ kind: "route" })}>删除本人路线</button></div>
        </div>{activeLayer && <div className="tactic-owned-items"><p>本人点位</p>{activeLayer.markers.filter((marker) => marker.canEdit).map((marker) => <button className="text-action" key={marker.id} disabled={!data.access.canDraw || busy} onClick={() => setConfirmation({ kind: "marker", marker })}>删除 {marker.type === "TEXT" ? marker.text : `(${marker.x.toFixed(2)}, ${marker.y.toFixed(2)})`}</button>)}</div>}</details>
        <ConfirmDialog open={confirmation?.kind === "layer"} title="删除当前战术图层？" description="该图层中的全部路线和点位都会永久删除，此操作无法撤销。" confirmLabel="永久删除图层" busy={busy} onClose={() => setConfirmation(null)} onConfirm={() => void removeLayer()} />
        <ConfirmDialog open={confirmation?.kind === "route"} title="删除本人路线？" description="当前图层中的本人路线会被删除，此操作无法撤销。" confirmLabel="删除路线" busy={busy} onClose={() => setConfirmation(null)} onConfirm={() => void removeOwnRoute()} />
        <ConfirmDialog open={confirmation?.kind === "marker"} title="删除本人点位？" description="这个点位或文字标记会被永久删除。" confirmLabel="删除点位" busy={busy} onClose={() => setConfirmation(null)} onConfirm={() => { if (confirmation?.kind === "marker") void removeMarker(confirmation.marker); }} />
    </section>
  </main>;
}
