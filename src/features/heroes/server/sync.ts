...
// ── Parse skins ─────────────────────────────────────────────────────
function parseDetailSkinNames(html: string): string | undefined {
...
async function fetchHeroList(cfg: CrawlConfig): Promise<HeroCatalogRecord[]> {
...
export async function syncHeroes(onProgress?: (p: SyncProgress) => void): Promise<{ inserted: number; updated: number }> {
  ...
  for (let b = 0; b < heroes.length; b += batchSize) {
    ...
    const results = await Promise.all(
      batch.map(async (h) => {
        ...
        const skins = createHeroSkins({
          heroId: h.ename,
          officialSkinNames: h.skin_name,
          detailSkinNames: html ? parseDetailSkinNames(html) : undefined,
          fallbackName: h.title || h.cname,
          skinImageTemplate: cfg.skin_img_base,
          heroImageTemplate: cfg.hero_img_base,
        });
    ...
    for (const { h, roleType, skills, skillsJson, skinsJson, imageUrl, dataHash, mingGe } of results) {
      ...
      const { processSkill } = await import("@/core/game");
  ...
  // 命格关系由确定 heroId 绑定；同步后自动恢复，避免依赖一次性迁移脚本。
  for (const pair of KNOWN_MINGGE_PAIRS) {
    const count = await prisma.hero.count({
      where: { heroId: { in: [pair.baseId, pair.formId] } },
    });
    if (count !== 2) continue;
    await prisma.$transaction([
      prisma.hero.update({
        where: { heroId: pair.baseId },
        data: {
          mingge: true,
          minggeName: pair.formName,
          minggeRelatedId: pair.formId,
        },
      }),
      prisma.hero.update({
        where: { heroId: pair.formId },
        data: {
          mingge: true,
          minggeName: null,
          minggeRelatedId: pair.baseId,
        },
      }),
    ]);
  }
...
// Self-execute only when run directly via the package sync-heroes script.
