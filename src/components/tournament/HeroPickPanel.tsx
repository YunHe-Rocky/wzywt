...
import { ROLE_LABELS, ROLE_COLORS } from "@/engine";
...
export function HeroPickPanel({
  ...
  // Load picks
  useEffect(() => {
    fetch(`/api/tournaments/${tournamentId}/picks`)
      .then(r => r.json()).then(d => setPicks(d.picks || []));
  }, [tournamentId]);
  ...
  useEffect(() => {
    ...
    Promise.all(toLoad.map(r =>
      fetch(`/api/heroes?role_type=${r}`).then(r => r.json())
    )).then(results => {
  ...
  const savePick = async (userId: number, heroId: number, team: string, roleType: string) => {
    const res = await fetch(`/api/tournaments/${tournamentId}/picks`, {
      ...
      body: JSON.stringify({ targetUserId: userId, heroId, team, roleType }),
    });
    if (res.ok) {
      setPicks(prev => {
...
function HeroMiniDetail({ heroId }: { heroId: number }) {
  ...
  useEffect(() => {
    fetch(`/api/heroes/${heroId}`).then(r => r.json()).then(setHero);
  }, [heroId]);
  if (!hero) return null;
