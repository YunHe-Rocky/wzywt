...
import { ROLE_LABELS, ROLE_COLORS, HERO_STAT_PROFILES, calcFinalStats, calcSkillDamage } from "@/engine";
import { HeroSelect } from "@/components/hero/HeroSelect";
import { EquipSelect } from "@/components/hero/EquipSelect";
...
export function TeamBuilder({
  ...
  useEffect(() => { fetch("/api/equipment").then(r => r.json()).then(items => { const m: typeof equipData = {}; for (const e of items) m[e.id] = { stats: e.stats }; setEquipData(m); }); }, []);

  useEffect(() => {
    fetch(`/api/tournaments/${tournamentId}/picks`).then(r => r.json()).then(d => {
      if (!d.picks?.length) return;
      const map: typeof picks = {};
      for (const p of d.picks) { map[p.userId] = { heroId: p.heroId, heroName: p.heroName, equipIds: p.equipJson || [] }; if (p.heroId > 0) fetch(`/api/heroes/${p.heroId}`).then(r => r.json()).then(h => { if (!h.error) setDetails(prev => ({ ...prev, [p.userId]: h })); }); }
      setPicks(map);
    });
  }, [tournamentId]);
  ...
  const save = (uid: number, data: { heroId?: number; heroName?: string; equipIds?: number[] }) => {
    ...
    fetch(`/api/tournaments/${tournamentId}/picks`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ targetUserId: uid, heroId: m.heroId, heroName: m.heroName, equipIds: m.equipIds }) }).catch(() => {});
  };

  const onHero = async (uid: number, heroId: string, heroName: string) => { save(uid, { heroId: parseInt(heroId), heroName }); const d = await fetch(`/api/heroes/${heroId}`).then(r => r.json()); setDetails(prev => ({ ...prev, [uid]: d })); };
            ...
            {!canEdit && pick?.equipIds && pick.equipIds.length > 0 && (
              <div style={{ display: "flex", gap: 4, marginBottom: 8 }}>
                {pick.equipIds.map(eid => {
                  ...
                  return e ? <img key={eid} title={e.stats?.map(s => `${s.stat}:${s.value}`).join(",") || ""}
