...
> 本页是早期表结构的阅读辅助，不是迁移或删除规则的权威来源。当前结构以 `prisma/schema.prisma` 和按时间排序的 `prisma/migrations/` 为准；生产只执行 `prisma migrate deploy`。运行时与 MySQL 兼容矩阵见 [维护指南](../operations/maintenance.md)。
