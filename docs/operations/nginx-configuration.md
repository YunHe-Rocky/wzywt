...
默认部署只信任同机 Nginx：边缘必须用 `$remote_addr` **覆盖** `X-Real-IP` 和 `X-Forwarded-For`，不能用会保留客户端自带首项的 `$proxy_add_x_forwarded_for`。应用限流只读取 Nginx 覆盖的 `X-Real-IP`；应用端口保持 loopback，避免外部请求绕过边缘伪造该头。

若前面还有 CDN 或多层代理，不要直接照搬单跳配置。先列出可信代理网段，在最外层验证来源并覆盖内部使用的客户端地址头，再验证伪造转发头不会改变限流归属。
