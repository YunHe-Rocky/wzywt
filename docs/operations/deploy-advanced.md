...
数据库备份默认写入 `<runtime>/shared/mysql-bak`。需要改到独立磁盘时，先由管理员创建只对部署用户开放的目录，再设置绝对路径；不要给整个 `/opt` 递归改权限：

```bash
sudo install -d -m 0700 -o project -g project /opt/middleware/Mysql/mysql/bakup/yanwutang
```

```dotenv
DEPLOY_DB_BACKUP_DIR=/opt/middleware/Mysql/mysql/bakup/yanwutang
```
